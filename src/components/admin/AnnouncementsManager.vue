<template>
  <div class="manager">
    ...
    <button @click="openAddForm" class="btn-add">
      {{ editingId ? 'Скасувати' : '+ Додати оголошення' }}
    </button>

    <div v-if="showForm" class="form-card">
      <h3>{{ editingId ? 'Редагувати оголошення' : 'Нове оголошення' }}</h3>
    ...
    <div class="items-list">
      <div v-for="item in sortedItems" :key="item.id" class="item-card" :class="{ important: item.important }">
        ...
        <div class="item-actions">
          <button @click="editAnnouncement(item)" class="btn-edit" title="Редагувати">✏️</button>
          <button @click="deleteAnnouncement(item.id)" class="btn-delete" title="Видалити">🗑️</button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async saveAnnouncement() {
      ...
      try {
        if (this.editingId) {
          await updateDoc(doc(db, 'announcements', this.editingId), data);
        } else {
...
.item-card.important {
  background: var(--important-bg, #fff5ed);
